...
contains
   ...
   subroutine do(self, _ARGUMENTS_DO_)
     ...
     _DECLARE_ARGUMENTS_DO_
      ...
      _LOOP_BEGIN_
         ...
         _ADD_SOURCE_(self%id_tempn,ftempn)
         ...
         _ADD_SOURCE_(self%id_tempsi,ftempsi)
         ...
         _ADD_SOURCE_(self%id_tempfe,ftempfe)
         ...
         _ADD_SOURCE_(self%id_tempc,ftempc)
         ...
         _ADD_SOURCE_(self%id_tempca,ftempca)
         _SET_DIAGNOSTIC_(self%id_FASTCA,ftempca * s_per_d)

         !LOCAL TRENDS

         ! chlorophyll
         _ADD_SOURCE_(self%id_ZCHN,((frn * fprn * ZPHN) - fgmipn - fgmepn - fdpn - fdpn2) * (fthetan / self%xxi))
         _ADD_SOURCE_(self%id_ZCHD,((frd * fprd * ZPHD) - fgmepd - fdpd - fdpd2) * (fthetad / self%xxi))

         ! phytoplankton
         _ADD_SOURCE_(self%id_ZPHN,(fprn * ZPHN) - fgmipn - fgmepn - fdpn - fdpn2 )
         _ADD_SOURCE_(self%id_ZPHD,(fprd * ZPHD) - fgmepd - fdpd - fdpd2 )
         _ADD_SOURCE_(self%id_ZPDS,(fprds * ZPDS) - fgmepds - fdpds - fsdiss - fdpds2 )

         _SET_DIAGNOSTIC_(self%id_OPAL, (fprds * ZPDS) * s_per_d)

         ! zooplankton
         _ADD_SOURCE_(self%id_ZZMI, fmigrow - fgmezmi - fdzmi - fdzmi2 )
         _ADD_SOURCE_(self%id_ZZME, fmegrow - fdzme - fdzme2 )

         ! detritus
         _ADD_SOURCE_(self%id_ZDET,fslown-fgmid-fgmed-fdd)
         ...
         _ADD_SOURCE_(self%id_ZDIN,fn_prod + fn_cons)
         ...
         _ADD_SOURCE_(self%id_ZSIL,fs_prod + fs_cons)

         ! dissolved iron
         _ADD_SOURCE_(self%id_ZFER, self%xrfn * (fn_prod + fn_cons))

        ! detrital carbon
         _ADD_SOURCE_(self%id_ZDTC, fslowc - fgmidc - fgmedc - fddc)
        ...
        ! dissolved inorganic carbon
         ...
         _ADD_SOURCE_(self%id_ZDIC,fc_prod + fc_cons)

         ! alkalinity
         fa_cons = -2._rk * ftempca      ! CaCO3 production

         _ADD_SOURCE_(self%id_ZALK, fa_cons)
         ...
         if (ZOXY .lt. self%xo2min) then                     ! suoxic zone, deficient O2; production fluxes only
            _ADD_SOURCE_(self%id_ZOXY, fo2_prod )
            ...
            _ADD_SOURCE_(self%id_ZOXY, fo2_prod + fo2_cons )
   ...
   subroutine do_bottom(self, _ARGUMENTS_DO_)
      ...
      _BOTTOM_LOOP_BEGIN_
         ...
         _ADD_BOTTOM_FLUX_(self%id_ZDET,-fluxn)
         _ADD_BOTTOM_FLUX_(self%id_ZDTC,-fluxc)

         _ADD_BOTTOM_SOURCE_(self%id_ZSEDC,  + fluxc)
         _ADD_BOTTOM_SOURCE_(self%id_ZSEDN,  + fluxn)
         _ADD_BOTTOM_SOURCE_(self%id_ZSEDFE, + fluxfe)

         _ADD_BOTTOM_SOURCE_(self%id_ZSEDP, + fluxc/106._rk)
