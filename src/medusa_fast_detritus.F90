...
!**************************************************************
! Note that fast-sinking detritus is created in medusa_pelagic module
!
module medusa_fast_detritus
   ...
   type, extends(type_particle_model), public :: type_medusa_fast_detritus
      ...
      type (type_bottom_diagnostic_variable_id) :: id_ffastc,id_ffastn,id_ffastsi,id_ffastfe,id_ffastca
      type (type_bottom_diagnostic_variable_id) :: id_SEAFLRN,id_SEAFLRSI,id_SEAFLRFE,id_SEAFLRC,id_SEAFLRCA
...
contains
   ...
   subroutine initialize(self, configunit)
      ...
      call self%register_diagnostic_variable(self%id_ffastca,'ffastca','mmol CaCO3 m-2 s-1','remineralisation of calcite (CaCO3)',missing_value=0.0_rk,source=source_do_column)

      call self%register_diagnostic_variable(self%id_SEAFLRN,'SEAFLRN','mmolN/m2/s','Seafloor flux of N')
      call self%register_diagnostic_variable(self%id_SEAFLRSI,'SEAFLRSI','mmolSi/m2/s','Seafloor flux of Si')
      call self%register_diagnostic_variable(self%id_SEAFLRFE,'SEAFLRFE','mmolFe/m2/s','Seafloor flux of Fe')
      call self%register_diagnostic_variable(self%id_SEAFLRC,'SEAFLRC','mmolC/m2/s','Seafloor flux of C')
      call self%register_diagnostic_variable(self%id_SEAFLRCA,'SEAFLRCA','mmolCaCO3/m2/s','Seafloor flux of CaCO3')
   ...
   subroutine do_column(self, _ARGUMENTS_DO_COLUMN_)
      ...
      !  4.  Remaining fast detritus is remineralised in the bottom box [subroutine do_bottom]
      !
      ...
      ! aside from explicitly modelled, slow-sinking detritus,
      ! the model includes an implicit representation of detrital
      ...
      ! ballast model more self-consistent
      !-----------------------------------------------------------------------------------------
      !
      fdep1 = 0._rk
      ccd_count = 0
      ...
      _DOWNWARD_LOOP_BEGIN_
        ...
        _SET_DIAGNOSTIC_(self%id_freminfe,freminfe * s_per_d)
         ...
         ! biogenic silicon
         fq0      = ffastsi                             ! how much opal enters this box             (mol)
         ...
         _GET_HORIZONTAL_(self%id_CAL_CCD,cal_ccd)      ! get calculated CCD field
      ...
      _SET_BOTTOM_DIAGNOSTIC_(self%id_ffastc,ffastc)
      _SET_BOTTOM_DIAGNOSTIC_(self%id_ffastn,ffastn)
      _SET_BOTTOM_DIAGNOSTIC_(self%id_ffastfe,ffastfe)
      _SET_BOTTOM_DIAGNOSTIC_(self%id_ffastsi,ffastsi)
      _SET_BOTTOM_DIAGNOSTIC_(self%id_ffastca,ffastca)
   ...
   subroutine do(self, _ARGUMENTS_DO_)
      ...
      ! Here fast detritus remineralised within the water column is added to inorganic nutrient pool
      ! with the corresponding modifications to oxygen and alkalinity.
      _LOOP_BEGIN_
         ...
         _ADD_SOURCE_(self%id_ZDIC, + freminc + freminca)
         _ADD_SOURCE_(self%id_ZDIN, + freminn * d_per_s)
         _ADD_SOURCE_(self%id_ZSIL, + freminsi * d_per_s)
         _ADD_SOURCE_(self%id_ZFER, + freminn * self%xrfn * d_per_s)

         if (ZOXY .ge. self%xo2min) _ADD_SOURCE_(self%id_ZOXY, - self%xthetarem * freminc - self%xthetanit * freminn * d_per_s)

         _ADD_SOURCE_(self%id_ZALK, 2._rk * freminca)
   ...
   subroutine do_bottom(self, _ARGUMENTS_DO_BOTTOM_)
      ...
      ! Three options for seafloor handling are implemented to recreate options offered by original MEDUSA
      ...
      ! seafloor.eq.3: (jfdfate.eq.0 .and. jorgben.eq.1) - conversion of fast detritus into benthic

      ! Note that opting for seafloor.eq.3 requires coupling of benthic module
      ! to allow for the mineralisation (and any transformations) of benthic material
      !
      _BOTTOM_LOOP_BEGIN_
         ...
         _GET_BOTTOM_(self%id_ffastsi1,ffastsi)
         _GET_BOTTOM_(self%id_ffastca1,ffastca)
         _GET_BOTTOM_(self%id_ffastfe1,ffastfe)

         _SET_BOTTOM_DIAGNOSTIC_(self%id_SEAFLRN,ffastn * s_per_d)
         _SET_BOTTOM_DIAGNOSTIC_(self%id_SEAFLRSI,ffastsi * s_per_d)
         _SET_BOTTOM_DIAGNOSTIC_(self%id_SEAFLRFE,ffastfe * s_per_d)
         _SET_BOTTOM_DIAGNOSTIC_(self%id_SEAFLRC,ffastc * s_per_d)
         _SET_BOTTOM_DIAGNOSTIC_(self%id_SEAFLRCA,ffastca * s_per_d)

         if (self%seafloor .eq. 1) then ! C remineralisation

            _ADD_BOTTOM_FLUX_(self%id_ZDIC, + ffastc)
            if (ZOXY .ge. self%xo2min) _ADD_BOTTOM_FLUX_(self%id_ZOXY, - self%xthetarem * ffastc - self%xthetanit * ffastn)
            _ADD_BOTTOM_FLUX_(self%id_ZDIC, + ffastca)
            _ADD_BOTTOM_FLUX_(self%id_ZALK, 2._rk * ffastca)
            _ADD_BOTTOM_FLUX_(self%id_ZDIN, + ffastn)
            _ADD_BOTTOM_FLUX_(self%id_ZSIL, + ffastsi)
            _ADD_BOTTOM_FLUX_(self%id_ZFER, + ffastn * self%xrfn)

         elseif (self%seafloor .eq. 2) then ! fast C -> slow C

            _ADD_BOTTOM_FLUX_(self%id_ZALK, 2._rk * ffastca)
            _ADD_BOTTOM_FLUX_(self%id_ZDTC, + ffastc)
            _ADD_BOTTOM_FLUX_(self%id_ZDET, + ffastn)
            _ADD_BOTTOM_FLUX_(self%id_ZSIL, + ffastsi)
            _ADD_BOTTOM_FLUX_(self%id_ZDIC, + ffastca)
         ...
         elseif (self%seafloor .eq. 3) then ! fast C -> benthic C
            ...
            _ADD_BOTTOM_SOURCE_(self%id_ZSEDC, + ffastc)
            _ADD_BOTTOM_SOURCE_(self%id_ZSEDN, + ffastn)
            _ADD_BOTTOM_SOURCE_(self%id_ZSEDSI, + ffastsi)
            _ADD_BOTTOM_SOURCE_(self%id_ZSEDCA, + ffastca)
            _ADD_BOTTOM_SOURCE_(self%id_ZSEDFE, + ffastfe)

            ! Benthic phosphate is added following the Redfield ratio, in case required by the benthic model
